...
try {
  ...
  for (let attempt = 0; attempt < 60; attempt++) {
    try { if ((await fetch('http://127.0.0.1:4399/')).ok) { ready = true; break } } catch {}
    ...
    try {
      ...
      await page.route('https://react-tweet.vercel.app/api/tweet/*', route => {
        ...
        return route.fulfill({ json: { data: {
          id_str: id, text: 'Playback fixture', display_text_range: [0, 16],
          created_at: '2026-09-21T00:00:00.000Z', user: { name: 'Fixture', screen_name: 'fixture' }, edit_control: {},
          video: { aspectRatio: [16, 9], mediaAvailability: { status: 'Available' }, videoId: { type: 'tweet', id },
            variants: [{ type: 'video/mp4', src: mediaUrl }] },
        } } })
      })
      for (const path of ['dashboard', 'preview']) {
        const response = await page.goto(`http://127.0.0.1:4399/playground/${path}/meowdown/main-editor/?doc=008-x-post`)
        ...
        assert.ok(requests.every(referer => referer === undefined), 'Media leaked Referer')
        const before = await player.evaluate(video => video.currentTime)
        await player.press('ArrowRight')
        await page.waitForFunction(time => document.querySelector('video')?.currentTime > time, before)
      }
      // The same origin rejects a document that sends its referrer.
      await page.route('http://127.0.0.1:4399/referrer-control', route => route.fulfill({
        contentType: 'text/html', body: `<video controls src="${mediaUrl}"></video>`,
      }))
      requests.length = 0
