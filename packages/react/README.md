...
- `bulletAfterHeading?: boolean`: pressing Enter at the end of the document's first heading (the title line) starts a fresh empty bullet on the next line instead of a plain paragraph; a heading anywhere else is left to the default Enter. Off by default. Ignored in source mode.
