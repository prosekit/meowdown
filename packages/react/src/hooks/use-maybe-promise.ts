 ...
 * otherwise `undefined` until it resolves. A rejection stays `undefined`. A new
 * `input` discards the previous `Promise`'s result at once, so a stale value
 * never shows for a newer input. Callers memoize `input`, or a fresh `Promise`
 * on every render never settles.
 */
