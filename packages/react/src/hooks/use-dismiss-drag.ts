...
/**
 * `tap`: a plain click or tap on the content. `drag`: the content was dragged
 * or flicked away, so it is already off screen.
 */
export type DismissReason = 'tap' | 'drag'

/**
 * Styles and handlers driving the touch drag-to-dismiss gesture.
 */
export interface DismissDrag {
  /**
...
/**
 * Touch drag-to-dismiss for the lightbox. A touch drag detaches
...
export function useDismissDrag({
  ...
  const release = useCallback(
    (event: ReactPointerEvent<HTMLElement>, interrupted: boolean): void => {
      const current = stateRef.current
      if (
        (current.phase !== 'armed' && current.phase !== 'dragging') ||
        current.pointerId !== event.pointerId
  ...
  const onPointerUp = useCallback(
    (event: ReactPointerEvent<HTMLElement>): void => release(event, false),
    [release],
  )

  const onPointerCancel = useCallback(
    (event: ReactPointerEvent<HTMLElement>): void => release(event, true),
    [release],
  )
