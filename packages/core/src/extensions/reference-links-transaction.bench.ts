...
test('ordinary edit', async ({ bench }) => {
  await registerScenario(bench, '1000 blocks, 20 definitions', () =>
    createOrdinaryEditScenario(1_000, 20),
  )
  await registerScenario(bench, '1000 blocks, 100 definitions', () =>
    createOrdinaryEditScenario(1_000, 100),
  )
  await registerScenario(bench, '1000 blocks, 400 definitions', () =>
    createOrdinaryEditScenario(1_000, 400),
  )
  await registerScenario(bench, '1000 blocks, 1000 definitions', () =>
    createOrdinaryEditScenario(1_000, 1_000),
  )
  await registerScenario(bench, '4000 blocks, 20 definitions', () =>
    createOrdinaryEditScenario(4_000, 20),
  )
  await registerScenario(bench, '16000 blocks, 1000 definitions', () =>
    createOrdinaryEditScenario(16_000, 1_000),
  )
})

test('definition keystroke', async ({ bench }) => {
  await registerScenario(bench, '1000 blocks, 100 dependents, 20 definitions', () =>
    createDefinitionEditScenario(1_000, 100, 20),
  )
  await registerScenario(bench, '4000 blocks, 400 dependents, 20 definitions', () =>
    createDefinitionEditScenario(4_000, 400, 20),
  )
  await registerScenario(bench, '1000 blocks, 1000 dependents, 20 definitions', () =>
...
test('reference index scan', async ({ bench }) => {
  ...
  await bench('1000 blocks, 20 definitions', () => {
    collectReferenceDefinitions(docWithTwentyDefinitions)
  }).run()
  await bench('1000 blocks, 1000 definitions', () => {
    collectReferenceDefinitions(docWithThousandDefinitions)
  }).run()
  await bench('4000 blocks, 20 definitions', () => {
    collectReferenceDefinitions(largeDoc)
  }).run()
})
