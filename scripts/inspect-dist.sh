...
#
# Usage: run it on a pull request branch that tracks a remote branch.
#
#   ./scripts/inspect-dist.sh
#
# It builds `origin/master` in a temporary worktree, commits its dist output
# to this branch, builds this branch, and commits its dist output on top. The
# last commit removes dist again, so the branch content is unchanged. Open the
# pull request's commit list on GitHub and read the `[dev] .js`, `[dev] .d.ts`
# and `[dev] .css` commits to see how the published files changed.
set -ex
...
# Clean up dist directories. Only touch dist: `cmk` rewrites the checked-in
# `*.module.d.css.ts` files during the build and those must not be committed here.
cd "$DEV_DIR"
git rm -r --cached 'packages/*/dist/' || true
rm -rf packages/*/dist || true
